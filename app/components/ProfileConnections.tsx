...
import { getFollowers, getFollowing, UserOut } from "../../lib/api";
import Avatar from "./Avatar";
import { Empty, ErrorBanner, Loading } from "./Feedback";

type ConnectionKind = "followers" | "following";

export default function ProfileConnections({ userId, kind, onClose }: { userId: number; kind: ConnectionKind; onClose: () => void }) {
  const [users, setUsers] = useState<UserOut[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  useEffect(() => {
    const load = kind === "followers" ? getFollowers(userId, { limit: 20, skip: 0 }) : getFollowing(userId, { limit: 20, skip: 0 });
