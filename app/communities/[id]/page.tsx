...
export default function CommunityDetailPage({ params }: { params: { id: string } }) {
  ...
  const toggleMembership = async () => { if (!community || joining) return; setJoining(true); setError(""); try { if (community.is_member) { await leaveCommunity(community.id); } else { await joinCommunity(community.id); } setCommunity({ ...community, is_member: !community.is_member, member_count: Math.max(0, (community.member_count || 0) + (community.is_member ? -1 : 1)) }); } catch (actionError) { setError(actionError instanceof Error ? actionError.message : "Unable to update membership."); } finally { setJoining(false); } };
