...
export default function CommunitiesPage() {
  ...
  return <div className="space-y-6">
    ...
    {loading ? <Loading label="Loading communities..." /> : communities.length === 0 ? <Empty title="No communities found" message="Try another search or create the first one." /> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{communities.map((community) => <Link className="vf-card vf-card-link p-5" href={`/communities/${encodeURIComponent(community.slug || String(community.id))}`} key={community.id}><p className="vf-eyebrow">Community</p><h2 className="mt-2 text-lg font-semibold">{community.name}</h2><p className="mt-2 line-clamp-3 text-sm text-slate-400">{community.description || "A new place for thoughtful conversation."}</p><span className="mt-5 block text-sm font-semibold text-[hsl(var(--accent))]">Open community →</span></Link>)}</div>}
  </div>;
}