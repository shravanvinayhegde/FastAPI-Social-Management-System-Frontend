...
export default function UsernameProfilePage({ params }: { params: { username: string } }) {
  ...
  const isOwnProfile = Boolean(profile?.actions.is_self);
  ...
  useEffect(() => {
    ...
    setTabLoading(true);
    if (activeTab === "media") {
      void getProfileMedia(username).then(setMedia).catch((tabError) => setError(tabError instanceof Error ? tabError.message : "Unable to load this tab.")).finally(() => setTabLoading(false));
      return;
    }
    if (activeTab === "replies") {
      void getProfileReplies(username).then(setReplies).catch((tabError) => setError(tabError instanceof Error ? tabError.message : "Unable to load this tab.")).finally(() => setTabLoading(false));
    } else {
      void getProfileLikes(username).then(setTabPosts).catch((tabError) => setError(tabError instanceof Error ? tabError.message : "Unable to load this tab.")).finally(() => setTabLoading(false));
    }
  ...
  const changeAvatar = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try { const next = await uploadAvatar(file); setProfile((current) => current ? { ...current, user: next.user, privacy: next.privacy, stats: next.stats, relationship: next.relationship, actions: next.actions } : current); }
  ...
  return <div className="mx-auto max-w-4xl space-y-5">
    ...
    <section className="vf-card overflow-visible p-0">
      <div className="flex flex-col gap-5 p-6 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">{profile.actions.is_self ? <button className="vf-btn-secondary" onClick={() => setEditing((value) => !value)}>{editing ? "Close" : "Edit profile"}</button> : <>{profile.actions.can_follow ? <FollowButton userId={profile.user.id} /> : null}{profile.actions.can_message ? <button className="vf-btn-secondary" onClick={() => void startMessage()} disabled={messaging}>{messaging ? "Opening..." : "Message"}</button> : null}</>}</div>
      </div>
